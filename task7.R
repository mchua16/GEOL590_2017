...
JFKonly_median %>% 
  arrange(desc(median)) #answer for JFK: 290, 300, 310, 330
...
f_a_median_JFKonly <- f_a_JFKonly %>% 
  ...
  arrange(desc(median_distance)) #arranges median distances from high to low
...
babynames_1896 <- babynames %>% 
  filter(year == 1896) %>%
  arrange(desc(prop)) #Ida,Pearl,Martha,Esther
...
babynames_1942 <- babynames %>% 
  filter(year == 1942) %>%
  arrange(desc(prop)) #Janice,Helen,Marilyn,Diane
...
babynames_2014 <- babynames %>% 
  filter(year == 2014) %>%
  arrange(desc(prop)) #Brookyln,Lily,Hannah,Layla (Did Drew mean 2014?)
